...
The exported game remains `offline`, `local_only` and device-authoritative: the SDK is present but receives no endpoint at normal boot. The next authority gate is a server-owned character snapshot/create/commit flow with ownership, revision, idempotency and conflict recovery. See `Notes/ONLINE_BACKEND_DECISION_2026-08-29.md` and `Notes/BACKEND_VERTICAL_SLICE_CONTRACT.md`.
