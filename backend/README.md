...
Status: reproducible local service and authoritative hunt/build economy are deployed and proven on public-TLS staging. Reusable, explicit-test-only Godot session and command boundaries now exercise ownership, archival cutover, read-only cache, reconnect, exact command retry and whole-unit economy/build/board revision recovery; every normal gameplay capability remains disabled pending Android lifecycle evidence and deliberate cutover.
