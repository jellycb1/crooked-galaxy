...
func _init() -> void:
	...
	check(ContentDB.procedural_collection_total() == 1940, "thirty-one authored planet packs expose the documented bounded 1940-series catalog")
	...
	horizon_rng.seed = 2400240
	var horizon_implant := ContentDB.generate_loot(ContentDB.TARGETS[104], horizon_rng, 3, "implant")
	check(str(horizon_implant.slot) == "implant" and str(horizon_implant.origin_planet_id) == "cartorio_ultimo_horizonte", "the level-240 pack produces canonical themed implant equipment")
	var university_rng := RandomNumberGenerator.new()
	university_rng.seed = 2500250
	var university_boots := ContentDB.generate_loot(ContentDB.TARGETS[108], university_rng, 3, "boots")
	check(str(university_boots.slot) == "boots" and str(university_boots.origin_planet_id) == "universidade_vilania_correspondencia", "the level-250 pack produces canonical themed boot equipment")
	var gods_rng := RandomNumberGenerator.new()
	gods_rng.seed = 2600260
	var gods_gloves := ContentDB.generate_loot(ContentDB.TARGETS[112], gods_rng, 3, "gloves")
	check(str(gods_gloves.slot) == "gloves" and str(gods_gloves.origin_planet_id) == "agencia_deuses_reformados", "the level-260 pack produces canonical themed glove equipment")
	var reserve_rng := RandomNumberGenerator.new()
	reserve_rng.seed = 2700270
	var reserve_helmet := ContentDB.generate_loot(ContentDB.TARGETS[116], reserve_rng, 3, "helmet")
	check(str(reserve_helmet.slot) == "helmet" and str(reserve_helmet.origin_planet_id) == "reserva_especies_impossiveis", "the level-270 pack produces canonical themed helmet equipment")
	var repair_shop_rng := RandomNumberGenerator.new()
	repair_shop_rng.seed = 2800280
	var repair_shop_rig := ContentDB.generate_loot(ContentDB.TARGETS[120], repair_shop_rng, 3, "rig")
	check(str(repair_shop_rig.slot) == "rig" and str(repair_shop_rig.origin_planet_id) == "oficina_realidades_defeituosas", "the level-280 pack produces canonical themed rig equipment")
