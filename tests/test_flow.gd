...
func _init() -> void:
	...
	check(int(summary.warp_chips) == 1 and int(state.player.warp_chips) == 1, "first completed mission of the UTC day grants one playable Warp Chip")
	check(int(state.player.daily_hunts_completed) == 1 and state.daily_rewards_ready() == 1, "a normal claimed hunt advances the daily shift exactly once")
	check(bool(summary.collection_new) and state.player.discovered_item_variant_ids.size() == 1, "claiming the first procedural item permanently discovers its template variant")
	...
	var chips_before_collection_claim := int(state.player.warp_chips)
	check(state.collection_rewards_ready() == 1 and state.claim_collection_milestone("series_1"), "first discovered series unlocks its explicit lifetime collection reward")
	check(int(state.player.warp_chips) == chips_before_collection_claim + 1 and state.last_notice_context == "collection", "collection reward credits the exact advertised premium amount")
	check(not state.claim_collection_milestone("series_1") and int(state.player.warp_chips) == chips_before_collection_claim + 1, "claimed collection reward cannot be paid twice")
