...
func _init() -> void:
	...
	source.player.helmet = {"id": "package_helmet", "name": "Capacete Afinado", "description": "Teste persistente.", "slot": "helmet", "power": 1, "rarity": "Raro", "color": "#58d9ff", "attribute_package_id": "reflex_tuning"}
	...
	check(str(restored.player.helmet.attribute_package_id) == "reflex_tuning", "canonical attribute packages survive save and load")
	...
	check(int(restored.current_bounty.get("scrap_reward", 0)) == 3 and int(restored.current_bounty.get("loot_power", 0)) == int(ContentDB.TARGETS[0].power), "corporate reward and canonical loot tier survive an interrupted capture")
	...
	check(int(atomic_reward.mastery_scrap) == 6 and int(restored.player.scrap) == reward_scrap_before + 9, "third corporate capture atomically grants contract and mastery scrap")
