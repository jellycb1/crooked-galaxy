...
func test_bounty_streak_rewards() -> void:
	...
	check(Rules.planet_tier_from_captures(2) == 0 and Rules.planet_tier_from_captures(3) == 1, "planet tiers advance every three captures")
	check(Rules.planet_next_tier_requirement(4) == 6 and Rules.planet_captures_until_next_tier(4) == 2, "planet progress exposes its next threshold and remaining captures")
	check(Rules.planet_next_tier_requirement(9) == -1 and Rules.planet_captures_until_next_tier(9) == 0, "maximum planet tier has no phantom requirement")
