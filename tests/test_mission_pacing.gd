...
func _init() -> void:
	...
	check(in_range(discovery, 27, 4400, 5200, 240), "the last-horizon-registry expansion arrives at its level-240 contract")
	check(in_range(discovery, 28, 4700, 5600, 250), "the correspondence-villainy-university expansion arrives at its level-250 contract")
	...
	check(in_range(discovery, 33, 6200, 7600, 300), "the bankrupt-empire-auction expansion arrives at its level-300 contract")
	check(in_range(discovery, 34, 6500, 8000, 310), "the industrial-coincidence-factory expansion arrives at its level-310 contract")
	check(in_range(discovery, 35, 6900, 8400, 320), "the cosmic-restart-center completes the level-320 launch catalog")
	...
	check(float(discovery[27].seconds) > float(discovery[26].seconds), "Last Horizon Registry extends the cumulative route ladder")
	check(float(discovery[28].seconds) > float(discovery[27].seconds), "Correspondence Villainy University extends the cumulative route ladder")
	...
	check(float(discovery[33].seconds) > float(discovery[32].seconds), "Bankrupt Empire Auction extends the cumulative route ladder")
	check(float(discovery[34].seconds) > float(discovery[33].seconds), "Industrial Coincidence Factory extends the cumulative route ladder")
	check(float(discovery[35].seconds) > float(discovery[34].seconds), "Cosmic Restart Center completes the cumulative route ladder")
