...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 19 and ContentDB.TARGETS.size() == 76, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 700:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 160 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 160 and exposes every implemented target identity")
	state.free()
	finish()
