...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 32 and ContentDB.TARGETS.size() == 128, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 5000:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 290 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 290 and exposes every implemented target identity")
	state.free()
	finish()
