...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 18 and ContentDB.TARGETS.size() == 72, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 400:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 150 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 150 and exposes every implemented target identity")
	state.free()
	finish()
