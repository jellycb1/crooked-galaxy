...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 35 and ContentDB.TARGETS.size() == 140, "the implemented planet axis fulfills the complete launch catalog")
	...
	for _hunt in 6500:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 320 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 320 and exposes every launch target identity")
	state.free()
	finish()
