...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 29 and ContentDB.TARGETS.size() == 116, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 3500:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 260 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 260 and exposes every implemented target identity")
	state.free()
	finish()
