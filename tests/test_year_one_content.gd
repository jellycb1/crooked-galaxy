...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 28 and ContentDB.TARGETS.size() == 112, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 3500:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 250 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 250 and exposes every implemented target identity")
	state.free()
	finish()
