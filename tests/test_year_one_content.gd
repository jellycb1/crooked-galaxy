...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 33 and ContentDB.TARGETS.size() == 132, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 5500:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 300 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 300 and exposes every implemented target identity")
	state.free()
	finish()
