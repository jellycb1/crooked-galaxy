...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 20 and ContentDB.TARGETS.size() == 80, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 1000:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 170 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 170 and exposes every implemented target identity")
	state.free()
	finish()
