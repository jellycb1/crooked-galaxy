...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 27 and ContentDB.TARGETS.size() == 108, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 3000:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 240 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 240 and exposes every implemented target identity")
	state.free()
	finish()
