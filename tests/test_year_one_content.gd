...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 34 and ContentDB.TARGETS.size() == 136, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 6000:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 310 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 310 and exposes every implemented target identity")
	state.free()
	finish()
