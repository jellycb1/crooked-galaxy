...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 31 and ContentDB.TARGETS.size() == 124, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 4500:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 280 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 280 and exposes every implemented target identity")
	state.free()
	finish()
