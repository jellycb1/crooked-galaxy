...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 23 and ContentDB.TARGETS.size() == 92, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 2000:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 200 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 200 and exposes every implemented target identity")
	state.free()
	finish()
