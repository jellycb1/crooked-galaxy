...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 24 and ContentDB.TARGETS.size() == 96, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 2000:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 210 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 210 and exposes every implemented target identity")
	state.free()
	finish()
