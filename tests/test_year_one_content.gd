...
func _init() -> void:
	...
	check(ContentDB.PLANETS.size() == 30 and ContentDB.TARGETS.size() == 120, "the audit distinguishes implemented planet packs from the launch catalog")
	...
	for _hunt in 4000:
		...
		if seen_targets.size() == ContentDB.TARGETS.size():
			break
	check(int(player.level) >= 270 and seen_targets.size() == ContentDB.TARGETS.size(), "one bounded post-year continuation reaches level 270 and exposes every implemented target identity")
	state.free()
	finish()
