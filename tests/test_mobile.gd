...
func run_mobile_audit() -> void:
	...
	scene.render()
	await process_frame
	check(find_label_with_text(scene, "HUNTER CAREER") != null and find_label_with_text(scene, "LEVEL 4 HUNTER") != null and find_label_with_text(scene, "7 CAPTURES · 2/25 KNOWN WORLDS") != null and find_label_with_text(scene, "NEXT LEVEL") != null and (scene.find_child("CareerProgressJump", true, false) as Button).text == "PROGRESS" and (scene.find_child("CareerArchiveJump", true, false) as Button).text == "WANTED · 100", "English career covers hunter progression, XP, discovered worlds, and section navigation")
