...
func run_mobile_audit() -> void:
	...
	check(scene.find_child("StateIndicator_toggle_on", true, false) != null and scene.find_child("SettingsLanguageOption_en", true, false) != null, "settings expose code-native toggle and radio state without incomplete raster controls")
