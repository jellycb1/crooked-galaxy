...
$Required = @(
    ...
    "local.yml", "package.json", "tsconfig.json", "src/main.ts", "src/generated_content.ts", "prepare_local_env.ps1", "test_local.ps1", "test_godot_client.ps1",
...
$GeneratedContent = Get-Content -LiteralPath (Join-Path $BackendRoot "src/generated_content.ts") -Raw
...
foreach ($RpcName in @("cg_clock", "cg_session", "cg_character_get", "cg_character_create", "cg_character_commit", "cg_economy_get", "cg_build_get", "cg_attribute_allocate", "cg_inventory_equip", "cg_inventory_recycle", "cg_hunt_board", "cg_hunt_accept", "cg_hunt_resolve", "cg_reward_claim")) {
...
foreach ($ManifestGuard in @('Generated by tools/generate_backend_content_manifest.gd', '"schema_version":1', '"content_hash":"', '"quiet_net"', '"central_reinicios_cosmicos"', '"cosmic_reset_administrator"')) {
    if (-not $GeneratedContent.Contains($ManifestGuard)) { throw "Generated backend content guard is missing: $ManifestGuard" }
}
foreach ($CombatGuard in @('validateCombatRuntime()', 'resolveCombat(active.combat_profile || current.profile, active)', 'combat_profile:', 'enemy_modifiers', 'profile.base_power', 'profile.stat_points')) {
    if (-not $RuntimeSource.Contains($CombatGuard)) { throw "Authoritative combat guard is missing: $CombatGuard" }
}
foreach ($Proof in @('idempotent_replay', 'status -ne "conflict"', 'credits = 999999', 'invalid_profile_change', 'approach_id = "quiet_net"', 'Resolved.result.won')) {
