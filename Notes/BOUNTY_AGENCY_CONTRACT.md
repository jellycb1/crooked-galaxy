...
`agency_apply` aceita apenas o ID de uma Agência e revisão social zero de um personagem sem membership. O servidor rejeita Agência inexistente, cheia, apenas por convite, membership ou candidatura prévia. Entrada `open` torna o personagem Agente imediatamente; modo `application` cria uma candidatura roster-free. Uma guarda server-only com lease e CAS serializa criação/candidatura por personagem; depois, um recibo é preparado antes da aresta Nakama e finalizado depois dela. Uma repetição exata pode recuperar uma interrupção, mas duas intenções simultâneas distintas nunca criam duas arestas. Nenhum cargo, nome de membro ou decisão de aprovação vem do candidato.
