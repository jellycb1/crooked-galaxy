...
The staging provider is now fixed as Hetzner: one disposable CX33 x86 cloud server in Helsinki, the currently available location shown by the operator console, plus a separate Falkenstein Storage Box for encrypted cross-region database archives. This is a staging choice, not a production-topology decision. A single CX33 provides no high availability and must never be represented as launch-ready production infrastructure.
