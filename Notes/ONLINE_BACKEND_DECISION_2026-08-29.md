...
The local server, Godot transport and character/economy authority slices are complete. The public Hetzner staging service, trusted TLS edge, encrypted off-host backup and isolated recovery drill are operational. An explicitly injected normal main-scene probe proves the official Godot client can authenticate a fresh disposable account, bind its pristine remote baseline to a non-importing local archive, verify ownership and clock, commit/replay/conflict, read exact economy/build state, accept a content-bound offer, reject premature resolution, remain active for the real 147-second starter duration, resolve/replay, claim a sealed item, equip it, protect it from recycling, persist a read-only cache and reconnect cleanly over public TLS without embedding the endpoint or client key in the APK. Physical Android certificate/lifecycle and real-save cutover confirmation remain before client activation. `account_backend`, `clock_backend`, `profile_backend`, `economy_backend`, `agency_backend` and billing remain false until their individual product gates pass; prepared infrastructure and test-only evidence alone enable none of them.
