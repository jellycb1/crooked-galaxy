...
A fresh installation now begins with a mandatory, resumable identity sequence before any game navigation appears: local login, explicit class confirmation, explicit species confirmation, and a validated 3–20 character hunter name. The login surface states honestly that this build uses a device-local test session rather than pretending to authenticate against an online service; session data is nevertheless persisted separately from character identity so a future account backend does not require another player-schema redesign. The initial cosmetic roster contains eight replaceable identities—Patched Terran, Discontinued Synthetic, Nebular Nomad, Cellar Mycelian, Rusted Ferrite, Tankborn Abyssal, Unstable Luminar, and Catalog Chimera. Species never alter attributes, combat, loot, economy, or progression. Each owns an original scalable emblem plus a distinct face palette and identifying marks on the shared procedural hunter portrait, carrying the choice through the final review, header, character sheet, and combat without adding bitmap residency. The naming step previews the assembled hunter and offers explicit class/species correction routes before final entry. Schema 12 preserves established campaign saves as returning local sessions, retains any selected class, and resumes at the first genuinely missing character choice. Every confirmation and correction is saved independently, corrupted-save recovery remains authoritative, and no default save silently invents class, species, or name.
