...
A fresh installation now begins with a mandatory, resumable identity sequence before any game navigation appears: locale, server, local login, explicit class confirmation, explicit species confirmation, and a validated 3–20 character hunter name. `International 1` is the first global multilingual shard and remains visible as `INT-1` in the compact game header. The current APK states honestly that its account is device-local and that no authoritative online connection exists yet. Portuguese is the complete selectable surface. The entire four-stage onboarding now uses stable translation keys and complete Portuguese/English PO catalogs, including all class/species narrative content, exact class mechanics, transaction feedback, and final identity review. English remains visibly unavailable until the same treatment reaches every playable game screen, preventing a mixed-language session. Locale and server live in account/session identity rather than the character profile, and the selected locale is applied through Godot's translation service. Schema 13 migrates established campaigns to Portuguese on International 1 without inventing an account for an interrupted fresh login. Unknown shards and incomplete locales are rejected at the state boundary.
