...
A fresh installation now begins with a mandatory, resumable identity sequence before any game navigation appears: locale, server, local login, explicit class confirmation, explicit species confirmation, and a validated 3–20 character hunter name. `International 1` is the first global multilingual shard and remains visible as `INT-1` in both compact headers. The current APK states honestly that its account is device-local and that no authoritative online connection exists yet. Portuguese is the complete selectable surface. The four-stage onboarding, persistent hunter/resource header, context-sensitive five-slot navigation, Frontier Menu, active planet/transport identity, device Settings, wanted board, all twenty target dossiers, three contract approaches, briefing, live hunt, all ten hunt incidents and thirty choices, automatic warrant combat, defeat diagnosis, victory handoff, complete normal-contract reward decision, Hunter sheet, five-attribute allocation, three-class comparison, universal equipment overview, inventory, loadouts, workshop, galaxy map, chapter-route state, travel receipt, planetary market, permanent transport hangar, Hunter Career, XP/level summary, offline patrol, eight milestones, mastery directive, planetary progress, and wanted archive now use stable translation keys backed by Portuguese/English PO catalogs. This includes every current target attack, player attack, combat effect, all 76 planet-catalog items, both starter items, all 21 item modifications, rarity and slot identities, equipment comparison, receipt, mastery, momentum, next-warrant projection, equip/store/recycle/repeat/workshop actions, filters, sorting, protection, field-test guidance, projected upgrades, market refresh/purchase, transport purchase/equip, milestone claims, AFK settlement, and localized transaction feedback. English remains visibly unavailable until Fenda and chapter finales receive the same treatment, preventing a mixed-language session. Locale and server live in account/session identity rather than the character profile, and the selected locale is applied through Godot's translation service. Schema 13 migrates established campaigns to Portuguese on International 1 without inventing an account for an interrupted fresh login. Unknown shards and incomplete locales are rejected at the state boundary.

The initial cosmetic roster contains eight replaceable identities—Patched Terran, Discontinued Synthetic, Nebular Nomad, Cellar Mycelian, Rusted Ferrite, Tankborn Abyssal, Unstable Luminar, and Catalog Chimera. Species never alter attributes, combat, loot, economy, or progression. Each owns an original scalable emblem plus a distinct face palette and identifying marks on the shared procedural hunter portrait, carrying the choice through the final review, header, character sheet, and combat without adding bitmap residency. The naming step previews the assembled hunter and offers explicit class/species correction routes before final entry. Every confirmation and correction is saved independently, corrupted-save recovery remains authoritative, and no default save silently invents class, species, or name.
...
Mandatory class selection now follows the same creation grammar. The focused provisional archetype uses its staged reference icon as a large preview, all three cards carry matching visual markers, and the screen states the primary attribute, preferred contract style, exact active specialization, and placeholder status before confirmation. The fixed footer keeps class commitment distinct from merely inspecting a card.
...
Completing Dustball also reveals the Fenda Clandestina, a six-floor one-time PvE ladder independent of planet warrants. It enters the existing animated automatic combat directly, shows deterministic build odds and the exact unique reward before entry, and advances only after the current floor's reward is claimed. Three visible class-neutral anomaly profiles repeat once under higher pressure: Câmara Volátil favors opening damage, Falha de Contenção largely pierces mitigation and rewards integrity, and Âncora Inercial preserves mixed builds. Their exact rule and intended build axis appear in the dossier before entry, keeping all three prototype classes viable without hidden class IDs or player-scaled enemies. Floors one through three introduce technical rigs; floors four through six introduce implants. These items follow the same universal one-to-two Power budget and modification contract as other secondary gear. Fenda wins grant fixed credits and XP but never increase warrant rank, planet captures, target mastery, or capture streak; defeats preserve both floor progress and the player's active warrant streak. The Career ledger now includes its own locked/current/completed Fenda track and direct route into the next floor, so parallel progression remains visible outside the Frontier Menu. Save schema 11 persists and sanitizes the current floor, canonical interrupted encounters, and unclaimed unique loot without allowing sequential floors to be skipped.
...
Three prototype bounty-hunter classes currently exercise that foundation: Quebra-Mandados specializes in Strength, Pistoleiro Orbital in Dexterity, and Hacker de Contratos in Intelligence. Their names, themes, copy, and final identities are explicitly replaceable placeholders; only their tested mechanical roles are current design scaffolding. Class effects are declared as data rather than ID-specific combat branches, so the roster can be replaced or expanded without rewriting combat. Every class adds one Power for every two points invested in its primary attribute and now owns an immediately active combat signature: Quebra-Mandados absorbs one base damage per enemy hit plus one for every two invested Strength; Pistoleiro Orbital gains 0.5% base persistent precision plus 0.25% per invested Dexterity, capped at 3%; Hacker de Contratos deals two base invasion damage plus one per invested Intelligence on the opening turn. The exact active signature appears in the selector, hunter sheet, affected turn card, and final combat report. Each selector previews the bonus produced by the hunter's current committed attributes without selecting or mutating the class. Vitality, Cunning, equipment, and every universal attribute effect remain active. Selection uses a separate mobile scroller and explicit confirmation; reclassification is free during early access so unfinished balance cannot trap an existing save.
...
`MENU DA FRONTEIRA` is the secondary service layer rather than a second copy of primary navigation. Its compact two-column grid contains Mercado, Hangar, Carreira, Fenda, and Ajustes; every destination pairs its emblem with a readable place name and a concise function or live state, followed by an illustrated current-planet and active-transport card. Android Back and visible `VOLTAR` actions preserve that hierarchy: services return to Menu, primary destinations return to Mandados, and Menu returns to the active contract board before exit. The icon-first grammar follows the reference's glanceable location principle without copying its imagery or adding bitmap residency. The five primary dock controls persist in place while their selected state changes, avoiding transient CanvasItem reconstruction on mobile GPUs.
...
The `HANGAR DUVIDOSO` adds four permanent transports unlocked across chapter completion. Bought models can be equipped freely and reduce only the approach's base hunt time by 10–40%; incident delays remain fully additive, while combat odds, rewards, loot power, and AFK patrols remain unchanged. Prices compete directly with market spending, turning the fastest models into campaign and post-campaign credit goals rather than compulsory combat upgrades. Ownership and the active model persist atomically through save schema 10, while malformed or prematurely unlocked records are repaired safely.
...
The galaxy map gives every chapter an original vector destination emblem without increasing texture memory: cratered Dustball, fractured Congelária ice, Micélia spores, Ferro-Velho machinery, and Cassino Quasar rings. Its compact location rows keep all five destinations visible together; current orbit, exact warrant progress, travel actions, completed chapters, and locked-route requirements remain identifiable without expanding every planet into a paragraph.
...
```powershell
powershell -ExecutionPolicy Bypass -File .\tools\publish_android_latest.ps1 -AllowPublicReferenceBuild
```

The command exports an ARM64 debug-signed APK and replaces only the asset on the `latest` release used by the team. Public upload requires the explicit `-AllowPublicReferenceBuild` switch authorized by the project owner; omitting it still fails closed. The dedicated ignored local test key keeps successive APKs update-compatible; it must never be reused for store distribution. The stable download address remains:
...
This channel is intended only for our direct testing. The project owner explicitly permits its current public accessibility while reference placeholders remain in the test APK. Preserve the ignored local test key to keep direct installs update-compatible; its non-secret public certificate fingerprint is tracked and verified on every export. A store release must use a separate private release keystore and signing workflow.
...
The proprietary study art in `References/` remains local, Git-ignored, and documented. The single Android test APK stages exactly thirteen registered images for composition testing on our own device: six environmental studies, two long-form interface surfaces, three provisional class emblems, one portrait frame, and one board divider. Market and hangar now have distinct temporary environments instead of sharing the arsenal location. Raw source files never enter Git or the package; only the explicit staged allowlist is exported, and every use and intended replacement is tracked in `Notes/REFERENCE_PLACEHOLDERS.md`.
...
- `References/` — external study material, excluded from Godot imports by `.gdignore`; the Android checker stages only the thirteen registered placeholders into the single test APK.
