...
Three original bounty-hunter classes now build on that foundation: Quebra-Mandados specializes in Strength, Pistoleiro Orbital in Dexterity, and Hacker de Contratos in Intelligence. A class adds one Power for every two points invested in its primary attribute, while Vitality, Cunning, equipment, and every universal attribute effect remain active. Selection uses a separate mobile scroller and explicit confirmation; reclassification is free during early access so unfinished balance cannot trap an existing save.
