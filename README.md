...
A fresh installation now begins with a mandatory, resumable identity sequence before any game navigation appears: locale, server, local login, explicit class confirmation, explicit species confirmation, and a validated 3–20 character hunter name. `International 1` is the first global multilingual shard and remains visible as `INT-1` in both compact headers. The current APK states honestly that its account is device-local and that no authoritative online connection exists yet. Portuguese and English are complete selectable surfaces. All current navigation, contracts, incidents, combat, rewards, Hunter/build management, galaxy, commerce, career, Fenda, five chapter finales, save/recovery feedback, and transaction receipts resolve through 1,199 parity-checked entries in each PO catalog. Language can be previewed at login and changed later in Settings; the account/session keeps the selected locale independently from class, species, and hunter name. An executable completeness guard rejects missing, empty, duplicated, or asymmetric catalog entries and renders every dynamic Fenda/finale family in English. Schema 13 migrates established campaigns to Portuguese on International 1 without inventing an account for an interrupted fresh login. Unknown shards and unavailable locales are rejected at the state boundary.
...
Completing Dustball also reveals the Fenda Clandestina, a six-floor one-time PvE ladder independent of planet warrants. It enters the existing animated automatic combat directly, shows deterministic build odds and the exact unique reward before entry, and advances only after the current floor's reward is claimed. Three visible class-neutral anomaly profiles repeat once under higher pressure: Câmara Volátil amplifies opening damage while piercing dampeners, Falha de Contenção disrupts opening attacks and targeting bonuses while rewarding sustained power, and Âncora Inercial preserves mitigation while amplifying stable class targeting systems. Their exact rule and intended build axis appear in the dossier before entry. The repeated profiles now form a guarded two-two-two matchup matrix: Hacker de Contratos leads both volatile floors, Quebra-Mandados leads both containment failures, and Pistoleiro Orbital leads both inertial anchors, while every balanced prototype build remains viable. No anomaly branches on a class ID or scales enemies from the player. Floors one through three introduce technical rigs; floors four through six introduce implants. These items follow the same universal one-to-two Power budget and modification contract as other secondary gear. Fenda wins grant fixed credits and XP but never increase warrant rank, planet captures, target mastery, or capture streak; defeats preserve both floor progress and the player's active warrant streak. The Career ledger now includes its own locked/current/completed Fenda track and direct route into the next floor, so parallel progression remains visible outside the Frontier Menu. Save schema 11 persists and sanitizes the current floor, canonical interrupted encounters, and unclaimed unique loot without allowing sequential floors to be skipped.
